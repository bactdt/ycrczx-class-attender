...
### 与原版差异
本仓库基于 `cxl086/ycrczx-class-attender` 调整，当前版本为 `1.0.9`。相对原版主要差异如下：
...
- 学习中心捕获到的未完成课程会保存为队列；当前课程真正结束且没有下一课时时，播放页会自动进入队列里的下一门未完成课程首课时。
...
- 播放页（`https://ycrczx.com/video/courseLearnPage?id=...&&classId=...`、`https://www.ycrczx.com/video/courseLearnPage?id=...&&classId=...`、`https://yctc.ycrczx.com/video/courseLearnPage?id=...&&classId=...`）：
  ...
  - 跨课程续学：若当前课程已到最后一课时，会读取学习中心保存的未完成课程队列，自动进入下一门课程。
