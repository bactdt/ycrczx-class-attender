...
### 与原版差异
本仓库基于 `cxl086/ycrczx-class-attender` 调整，当前版本为 `1.0.4`。相对原版主要差异如下：
...
- 视频结束后会暂停强制续播原视频，并增强“下一节”按钮和课程目录链接识别，减少停留在原视频末尾反复播放的问题。
...
- 播放页（`https://ycrczx.com/video/courseLearnPage?id=...&&classId=...`、`https://www.ycrczx.com/video/courseLearnPage?id=...&&classId=...`、`https://yctc.ycrczx.com/video/courseLearnPage?id=...&&classId=...`）：
  ...
  - 结束后跳转保护：视频已结束时不再强制播放原视频，并会从页面课程目录中查找当前课时后的下一个播放链接。
